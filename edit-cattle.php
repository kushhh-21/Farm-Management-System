...
<?php 
 if(!$_GET['id'] OR empty($_GET['id']) OR $_GET['id'] == '')
 {
 	...
 	$cattleno = $weight = $gender = $remark = $arr = $bname = $b_id = $health = $img = "";
 	$id = (int)$_GET['id'];
...
<div class="w3-container" style="padding-top:22px">
 <div class="w3-row">
     ...
     <?php
      if(isset($_POST['submit']))
      {
      	...
      	$update_query = $db->query("UPDATE cattles SET cattleno = '$n_cattleno',weight = '$n_weight',arrived = '$n_arrived', breed_id = '$n_breed', remark = '$n_remark',health_status = '$n_status' WHERE id = '$n_id' ");
 ...
 <div class="col-md-4 col-md-offset-2">
 	...
 	<a class="btn btn-danger btn-md" onclick="return confirm('Continue delete cattle ?')" href="delete.php?id=<?php echo $id ?>"><i class="fa fa-trash"></i> Delete Cattle</a>
 </div>
</div>
</div>
</div>
